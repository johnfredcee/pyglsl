...
parser = argparse.ArgumentParser(description="Read FBX file and dump to output.")
parser.add_argument('--fbx', metavar='F', nargs=1)
args = parser.parse_args()

sys.path = sys.path + [os.curdir + os.sep + "fbx2012_1"]    
try:
	from FbxCommon import *
except ImportError:
	print("You need to builld the fbx 2020_1 python bindings and put them in the fbx2020_1 folder")
	sys.exit(1)
...
def DisplayMetaData(pScene):
	...
	if sceneInfo:
		...
		if thumbnail:
			...
			if thumbnail.GetDataFormat() == fbx.FbxThumbnail.eRGB_24 :
				print("        Format: RGB")
			elif thumbnail.GetDataFormat() == fbx.FbxThumbnail.eRGBA_32:
				print("        Format: RGBA")	

			if thumbnail.GetSize() == fbx.FbxThumbnail.eNOT_SET:
				print("        Size: no dimensions specified (%ld bytes)", thumbnail.GetSizeInBytes())
			elif thumbnail.GetSize() == fbx.FbxThumbnail.e64x64:
				print("        Size: 64 x 64 pixels (%ld bytes)", thumbnail.GetSizeInBytes())
			elif thumbnail.GetSize() == fbx.FbxThumbnail.e128x128:
...
def DisplayNodeContent(pNode):
	if pNode.GetNodeAttribute() == None:
		...
		if lAttributeType == fbx.FbxNodeAttribute.eMarker:
			DisplayMarker(pNode)
		elif lAttributeType == fbx.FbxNodeAttribute.eSkeleton:
...
def InterpolationFlagToIndex(flags):
	...
	if (flags&KFCURVE_INTERPOLATION_LINEAR)==KFCURVE_INTERPOLATION_LINEAR:
	   return 2
	if (flags&FCURVE_INTERPOLATION_CUBIC)==KFCURVE_INTERPOLATION_CUBIC:
	   return 3
	return 0
