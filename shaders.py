...
class Shader:
	...
	def createShader(self, strings, type):
		...
		pyglet.gl.glShaderSource(shader, 1, cffi.byref(c_source_ptr), None)
	...
	def link(self):
		pyglet.gl.glBindFragDataLocation(self.handle, 0, b"fcolor")
		# link the program
	...
	def enumerate_attributes(self):
		...
		for i in range(0, self.attributeCount):
			...
			print("Attrib size %d, type %04x, name %s " %  (attrib_size.value, attrib_type.value, cname.value))
	...
	def enumerate_uniforms(self):
		...
		for i in range(0, self.uniformCount):
			...
			print("Uniform  location %d size %d, type %04x, name %s " % (location, uniform_size.value, uniform_type.value, cname.value))
